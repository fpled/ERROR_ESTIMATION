...
/// ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
template<class TM>
unsigned match_node_to_vertex_node( const TM &m, Vec<bool> &correspondance_node_to_vertex_node, Vec<unsigned> &connect_node_to_vertex_node, const bool disp = false ) {
...
/// -----------------------------------------------------------------------------------------------------------
template<class TM, class TF>
void construct_node_type( TM &m, const TF &f, const Vec< Vec<unsigned> > &face_type, Vec< Vec<unsigned> > &node_type, const bool disp = false ) {
    ...
    construct_faces_connected_to_node( m, face_cpt_node, face_list_node, disp );
...
/// ------------------------------------------------------------------
template<class TM, class TF>
void display_mesh_connectivity( TM &m, const TF &f, const bool disp = true ) {
    ...
    construct_child( m, child_cpt, child_list, disp );

    Vec<unsigned> elem_cpt_node;
    Vec< Vec<unsigned> > elem_list_node;
    construct_elems_connected_to_node( m, elem_cpt_node, elem_list_node, disp );
    ...
    construct_nodes_connected_to_elem( m, node_cpt_elem, node_list_elem, disp );

    Vec<unsigned> node_cpt_face;
    Vec< Vec<unsigned> > node_list_face;
    construct_nodes_connected_to_face( m, node_cpt_face, node_list_face, disp );

    Vec<bool> correspondance_node_to_vertex_node;
    Vec<unsigned> connect_node_to_vertex_node;
    unsigned nb_vertex_nodes = match_node_to_vertex_node( m, correspondance_node_to_vertex_node, connect_node_to_vertex_node, disp );

    Vec<unsigned> elem_cpt_vertex_node;
    Vec< Vec<unsigned> > elem_list_vertex_node;
    construct_elems_connected_to_vertex_node( m, nb_vertex_nodes, correspondance_node_to_vertex_node, connect_node_to_vertex_node, elem_cpt_vertex_node, elem_list_vertex_node, disp );

    Vec<unsigned> face_cpt_vertex_node;
    Vec< Vec<unsigned> > face_list_vertex_node;
    construct_faces_connected_to_vertex_node( m, nb_vertex_nodes, correspondance_node_to_vertex_node, connect_node_to_vertex_node, face_cpt_vertex_node, face_list_vertex_node, disp );

    Vec<unsigned> vertex_node_cpt_elem;
    Vec< Vec<unsigned> > vertex_node_list_elem;
    construct_vertex_nodes_connected_to_elem( m, correspondance_node_to_vertex_node, connect_node_to_vertex_node, vertex_node_cpt_elem, vertex_node_list_elem, disp );

    Vec<unsigned> vertex_node_cpt_face;
    Vec< Vec<unsigned> > vertex_node_list_face;
    construct_vertex_nodes_connected_to_face( m, correspondance_node_to_vertex_node, connect_node_to_vertex_node, vertex_node_cpt_face, vertex_node_list_face, disp );
    ...
    construct_node_type( m, f, face_type, node_type, disp );
