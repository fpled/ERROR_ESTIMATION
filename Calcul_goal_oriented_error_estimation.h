...
void calcul_enhanced_local_error_bounds( TM &m, TM &m_adjoint, const TF &f, const TF &f_adjoint, TM &m_lambda_min, TM &m_lambda_max, TM &m_lambda_opt, TM &m_adjoint_lambda_min, TM &m_adjoint_lambda_opt, const unsigned &deg_p, const string &method, const string &method_adjoint, const string &structure, const string &loading, const string &mesh_size, const unsigned &cost_function, const bool &enhancement_with_geometric_criterium, const bool &enhancement_with_estimator_criterium, const T &val_geometric_criterium, const T &val_estimator_criterium, const string &geometric_criterium, const unsigned &deg_k, const string &local_improvement, const string &shape, const T &k_min, const T &k_max, const T &k_opt, const string &interest_quantity, const string &direction_extractor, const string &pointwise_interest_quantity, const Vec<unsigned> &list_elems_interest_quantity, const unsigned &node_interest_quantity, const Pvec &pos_interest_quantity, const Pvec &pos_crack_tip, const T &radius_R1, const T &radius_R2, const bool &spread_cut, const T &theta, const T &theta_adjoint, const Vec<T> &theta_adjoint_elem, const Vec<unsigned> &correspondance_elem_m_adjoint_to_elem_m, const Vec< Vec<T> > &dep_hat, const Vec< Vec<T> > &dep_adjoint_hat, const T &I_h, const T &I_hh, const string &integration_k, const unsigned &integration_nb_steps, const bool &debug_method, const bool &debug_method_enhancement, const bool &debug_geometry, const bool &debug_error_estimate, const bool &want_introduction_sigma_hat_m, const bool &want_compute_eig_local_improvement, const bool &use_mask_eig_local_improvement ) {
    ...
    construct_center_length_domain( m, deg_p, shape, interest_quantity, list_elems_interest_quantity, node_interest_quantity, pos_crack_tip, radius_R2, domain_center, domain_length );

    /// Calcul de la constante impliquee dans l'amelioration
    ///-----------------------------------------------------
    
    T h = 0.;
    if ( want_compute_eig_local_improvement == 0 ) {
        cout << "-----------------------------------------------------------------" << endl;
        cout << "Calcul analytique de la constante h impliquee dans l'amelioration" << endl;
        cout << "-----------------------------------------------------------------" << endl << endl;
        ...
        cout << "constante h impliquee dans l'amelioration :" << endl;
        cout << "h = " << h << endl << endl;
    }
    else {
        cout << "----------------------------------------------------------------" << endl;
        cout << "Calcul numerique de la constante h impliquee dans l'amelioration" << endl;
        cout << "----------------------------------------------------------------" << endl << endl;
...
//                 display( m_mask, shape );
                ...
                cout << "constante h impliquee dans l'amelioration calculee numeriquement :" << endl;
                cout << "h = " << h << endl << endl;
