...
void construc_standard_force_fluxes_EET_PGD( TM &m, TF &f, const string &pb, const unsigned &cost_function, const bool enhancement, const Vec<bool> &flag_face_enh, const string &solver_minimisation, Vec< Vec< Vec<T> > > &force_fluxes, const TTVV &dep_space, const TTVVV &dep_param, const TVV &dep_part, const TTVV &kappa, const TMATVV &K_param, const Vec< Vec<unsigned> > &elem_group, const unsigned &mode, const bool want_local_enrichment = false, const bool verif_solver_minimisation = false, const T tol_solver_minimisation = 1e-6, const bool verif_compatibility_conditions = false, const T tol_compatibility_conditions = 1e-6, const bool disp = false ) {
        ...
        for (unsigned i=0;i<m.node_list.size();++i) {
            for (unsigned d=0;d<dim;++d) {
                if ( node_type[ i ][ d ] == 2 ) {
                    ...
                    if ( fabs( sum_r[ i ][ d ] - sum_q[ i ][ d ] ) > tol_compatibility_conditions ) {
                        cout << "verification des conditions de compatibilite au noeud " << i << " dans la direction " << d << " :" << endl;
